> 現行版1.2の追加仕様・変更点は[リリースガイド](RELEASE_1_2.md)と[公開スキーマ](../public/schemas/frame-v2.schema.json)を参照してください。本書の旧localStorage・未実装表記は1.2の記述で置き換えます。
...
- 保存時スナップショットとの差分で dirty 判定。ただし表示状態であるアクティブ荷重ケースは除外し、Undo / Redoでも現在の表示ケースを維持
...
画面からのユーザーJSON読込はstrictを先に実行し、型違反時だけlenientで再試行して互換読込の警告と各補正診断を表示します。数値文字列、整数への切捨て、一般的な真偽値の `1 / 0 / true / false` 表現をlenientで変換します。戻り値は現行 `formatVersion`、移行元バージョン、診断一覧です。未知の将来バージョンは拒否し、対象ドキュメントを変更しません。
...
複数行選択、Ctrl / Shift 選択、元データ添字を維持する検索・列フィルター・行フィルター・表示ソート、行スクロール、TSV コピー＆ペーストを提供します。貼り付けの空セルは列型にかかわらず既存値を維持してスキップし、実際に変更した複数セルだけを1件の構造化イベントで通知します。表示ソートは番号再採番とは別操作です。
