...
- **ファイル入出力** — シンプルな JSON 形式で保存・読込。解析用 YAML のインポートにも対応
...
|------|------|
| 新規作成 | 空のモデルを作成 |
| 開く | `.json` ファイルを読み込み、または解析用 `.yaml` / `.yml` ファイルを形状確認用にインポート |
...
│   └── FrameAnalysisYaml.ts # 解析用 YAML インポート
├── viewer/
...
`schema_version`, `units`, `model` を持つ解析用 YAML を、形状確認用のフレームモデルとして読み込めます。読み込み時に `mm/N` 系の値はアプリの `cm/kN` 系へ変換され、節点、境界条件、材料、断面、線材要素が現在の編集モデルへ展開されます。

YAML はインポート専用です。保存操作では従来どおり JSON が出力されます。`equalDOF`、節点質量、グループ、トレース情報、`twoNodeLink3D` の剛性・方向など、現行 JSON に保存先がない解析情報は完全には保持されません。読み込み時にスキップまたは簡略化された要素は、ステータスとコンソール診断で確認できます。
