...
export function parseStructForm(text, doc) {
    doc.init();
    ...
    const matDict = new Map();
    const springDict = new Map();
    const sectionDict = new Map();
    const memberDict = new Map();
    // デフォルトバネを辞書に追加
    ...
    while (cursor < lines.length) {
        ...
        if (node)
            node.boundaryCondition = bound;
    }
    // MATERIAL → M-MATERIAL まで
    while (cursor < lines.length) {
        const line = nextLine();
        if (line.trim() === 'M-MATERIAL')
            break;
        const split = line.split(',');
        if (split.length < 6)
            continue;
    ...
    while (cursor < lines.length) {
        ...
        sec.type = parseInt_(split[2]);
        sec.shape = parseInt_(split[3]);
    ...
    while (cursor < lines.length) {
        ...
        if (trimmed === 'WALL')
            break;
        if (trimmed === 'AI-LOAD') { /* skip to LOAD-DEFINITION */
            break;
        }
    ...
    if (prevTrimmed === 'AI-LOAD' || lines[cursor - 1]?.trim() === 'AI-LOAD') {
        ...
        while (cursor < lines.length) {
            ...
            if (line.trim() === 'CALCULATION-CASE' || line.trim() === 'STOP')
                break;
            nextLine();
        }
    }
    ...
    while (cursor < lines.length) {
        ...
        if (headerLine.trim() === 'CALCULATION-CASE' || headerLine.trim() === 'STOP')
            break;
        // caseIndex を更新
        ...
        if (cursor < lines.length) {
            ...
            if (check === 'CALCULATION-CASE' || check === 'STOP')
                break;
        }
    }
...
//# sourceMappingURL=StructFormParser.js.map