...
export class FrameDocument {
    ...
    /** 番号から節点を検索 */
    findNodeByNumber(number) {
        return this.nodes.find(n => n.number === number);
    }
    /** 番号からバネを検索 */
    findSpringByNumber(number) {
        if (number === Spring.RIGID.number)
            return Spring.RIGID;
        if (number === Spring.PIN.number)
            return Spring.PIN;
    ...
    addLoadCase() {
        this.loadCaseCount++;
        for (const n of this.nodes)
            n.setLoadCaseCount(this.loadCaseCount);
        for (const m of this.members)
            m.setLoadCaseCount(this.loadCaseCount);
        this.notifyChange();
    }
    /** 荷重定義を削除 */
    removeLoadCase(index) {
        if (index < 0 || index >= this.loadCaseCount)
            return;
        for (const n of this.nodes)
            n.removeLoad(index);
        for (const m of this.members)
    ...
    /** 重複ノード統合 */
    mergeOverlappingNodes(threshold) {
        ...
        for (const m of this.members) {
            ...
            if (iNode && deleted.has(iNode)) {
                ...
                if (replacement)
                    m.iNodeNumber = replacement.number;
            }
            if (jNode && deleted.has(jNode)) {
                const replacement = mergeMap.get(jNode);
                if (replacement)
                    m.jNodeNumber = replacement.number;
            }
        }
        this.notifyChange();
    }
}
//# sourceMappingURL=FrameDocument.js.map